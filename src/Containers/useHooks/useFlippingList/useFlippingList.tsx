...
const useFlippingList = ({
  ...
  useEffect(() => {
    ...
    if (indxList < arrList.length) {
      setFlippingList(arrList.slice(arrStart, arrStart + yardage));
      setFlippingListIndx(arrStart + yardage);
    } else {
      setFlippingList(arrList.slice(arrStart - yardage, arrStart));
      setFlippingListIndx(arrStart - yardage);
    }
  ...
  const FlippingListForth = () => {
    if (flippingListIndx < arrList.length) {
      setFlippingList(
        arrList.slice(flippingListIndx, flippingListIndx + yardage)
      );
  ...
  const FlippingListBeck = () => {
    if (flippingListIndx > yardage) {
      setFlippingList(
        arrList.slice(
          flippingListIndx - 2 * yardage,
          flippingListIndx - yardage
        )
      );
      setFlippingListIndx(flippingListIndx - yardage);
    }
